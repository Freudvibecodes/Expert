...
const MODALITIES = [
  "SFBT", "CBT", "Narrative Therapy", "Adlerian Therapy",
  "Structural Family Therapy", "Person-Centred Therapy", "DBT", "ACT",
  "Psychodynamic Therapy", "EFT", "Gestalt Therapy",
  "Motivational Interviewing", "EMDR", "Schema Therapy",
  "Existential Therapy", "Integrative", "Other",
];

const ISSUES = [
  "Randomised — surprise me", "Anxiety", "Depression", "Grief and loss",
  "Relationship conflict", "Family conflict", "Trauma", "Low self-esteem",
  "Life transitions", "Addiction", "Anger management",
  "Identity and belonging", "Work and burnout", "Procrastination",
  "People-pleasing / boundary difficulties", "Chronic guilt", "Perfectionism",
  "Health anxiety", "Social isolation / loneliness", "Imposter syndrome",
...
- You supervise the WHOLE session — modality, direction, pacing, silence, emotional depth, question types, intervention choices, and any in-the-moment clinical decision. Not just modality adherence.
- When a student asks whether to go deeper, stay on topic, change direction, hold silence, or any other moment — guide them to think it through themselves.
- Never answer clinical questions directly. Ask the next right question that helps them find the answer.
- One question per response. One short reframe sentence if truly needed. Four sentences maximum.
- If reasoning is on the right track, briefly validate it then deepen with a question.
...
Rules:
1. Reveal your presenting problem gradually — in small natural pieces over the session, the way a real client would. Never dump it all at once.
...
function SetupScreen({ onStart }) {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="tabs">
          <button className={`tab${mode === "group" ? " active" : ""}`} onClick={() => setMode("group")}>
            Group Supervision
          </button>
        ...
        {mode === "solo" && (
          <>
            <p style={{ fontSize: "0.875rem", color: "var(--text2)", marginBottom: "1.25rem", lineHeight: 1.7 }}>
              Practice solo. Claude plays a real client — revealing the presenting problem gradually, just like a real session. Receive a full clinical review when you close the session.
            </p>
            <div className="field">
            ...
            <div className="field">
              ...
              <select value={sRespMode} onChange={e => setSRespMode(e.target.value)}>
                <option value="voice">Voice — Claude speaks responses aloud</option>
                <option value="text">Text — Claude responds in writing</option>
              </select>
            </div>
          </>
        )}
    ...
    <div>
      <div className="card">
        <div className="row">
          ...
          <button className="btn btn-sm danger" onClick={() => { activeRef.current = false; try { recRef.current?.stop(); } catch (e) {} onEnd(transcriptRef.current.map(l => l.text).join("\n")); }}>
            End &amp; review
          </button>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="hint" style={{ marginTop: "0.5rem" }}>
          Say <strong>"Hey Claude"</strong> followed by your question at any point.
        </div>
      </div>
  ...
  useEffect(() => {
    async function init() {
      try {
        const reply = await callAPI("/api/chat", {
          system: buildClientSystem(config.modality, config.issue, personalityRef.current),
          messages: [{ role: "user", content: "The therapist has just welcomed you in. You have just sat down. You have not spoken yet — wait for them to begin. Just a brief natural settling-in moment." }],
        });
        addLine(reply, "client");
        setClientReply(reply);
        setDotState("client");
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        {interim && <div className="interim">{interim}</div>}
        <div className="hint" style={{ marginTop: "0.5rem" }}>
          Speak naturally — the microphone is always on. The client responds after you pause.
        </div>
      </div>
          ...
          {conversation.length === 0
            ...
            : conversation.map(l => (
              <div key={l.id} className={`t-line ${l.role === "therapist" ? "therapist" : "client"}`}>
                <span style={{ opacity: 0.5, fontSize: "0.75rem", marginRight: "0.4rem" }}>
...
function ReviewScreen({ config, transcript, onReset }) {
  ...
  async function generate(mode) {
    ...
    try {
      ...
      setReviewText("Connection issue generating review — please try again.");
      setStep("done");
    }
  }
...
export default function Home() {
  const [screen, setScreen] = useState("setup"); // setup | group | solo | review
